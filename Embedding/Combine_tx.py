...
key_words = ["origin"]
pattern = re.compile(r"^\s*(require|if)\s*\(\s*tx\.origin\s*[!=]=?\s*[\w\d_]+\s*\)", re.MULTILINE)
########################################################################################################################################
...
for doc in corpus:
    ...
    # Check regex result and handle 'origin'.
    if regex_result[counter] == 1:
        word = 'origin'
        if word in word2vec_model.wv:
            word_vector_tfidf[word] = word2vec_model.wv[word]
        ...
        for word_id, tfidf_score in tfidf_model[doc]:
            # Find the word with highest TF-IDF score.
            if tfidf_score > max_tfidf_score:
                max_tfidf_score = tfidf_score
                max_word_id = word_id
    
        # If the word was found, extract the word vector.
        if max_word_id is not None:
            word = dictionary[max_word_id]
            if word in word2vec_model.wv:
                word_vector_tfidf[word] = word2vec_model.wv[word]
    counter += 1
'''
...
with open('../Contracts/vuln/tx/mix/document_vectors.pkl', 'wb') as f:
    pickle.dump(document_vectors, f)
    
word2vec_model.save("../Contracts/vuln/tx/mix/word2vec_model.model")