...
def scan_for_reentrancy(file_path):
    # Regex pattern updated to handle multiple whitespace and potential newlines
    # between expressions and statements
    pattern = r'msg.sender\s*\.call\s*\.value\((.+?)\)\s*\(\)(.|\n)+?(balances?\s*\[\s*msg.sender\s*\]\s*-=)'
...
for i in range(1, 82):
    input_file_path = f"/home/antiransom/Unsupervised/Contracts/vuln/reentrancy/{i}.sol"
    output_file_path = f"/home/antiransom/Unsupervised/Contracts/vuln/reentrancy/{i}.txt"

    clean_and_save_sol_file(input_file_path, output_file_path)
    print(f"Processed file {i}")
    
    #file_path = f"/home/antiransom/Unsupervised/Contracts/vuln/reentrancy/{i}.sol"
    results = scan_for_reentrancy(input_file_path)
    print("Potential Reentrancy Issues Found:", results)
