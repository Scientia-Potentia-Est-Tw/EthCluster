...
def scan_files_up_to_n(n, directory_path):
    for i in range(1, n + 1):
        file_name = f"RE_{i}.sol"
        file_path = os.path.join(directory_path, file_name)
        result = detect_patterns_in_file(file_path)
        print(f"File {file_name}: {result}")

# Example usage
directory_path = '/home/antiransom/Unsupervised/Contracts/vuln/Dataset/reentrancy (RE)'  # Set the directory path where the files are located
